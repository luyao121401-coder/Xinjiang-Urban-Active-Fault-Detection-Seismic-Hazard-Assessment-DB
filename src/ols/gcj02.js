...
export var gcjMecator = () => {

  const gcj02Extent = [-20037517.342789244, -20037508.342789244, 20037499.342789244, 20037508.342789244];
  var gcjMecator = new Projection({
    code: "GCJ-02",
    extent: gcj02Extent,
    units: "m"
  })
...
export var projts = () => {
  ...
  var projts = {};
  projts.ll2gmerc = function(input, opt_output, opt_dimension) {
    let output =gcj02.fromWGS84(input, opt_output, opt_dimension);
    return projts.ll2smerc(output, output, opt_dimension);
  };
  projts.gmerc2ll = function(input, opt_output, opt_dimension) {
    let output = projts.smerc2ll(input, input, opt_dimension);
    return gcj02.toWGS84(output, opt_output, opt_dimension);
  };
  projts.smerc2gmerc = function(input, opt_output, opt_dimension) {
    let output = projts.smerc2ll(input, input, opt_dimension);
    output = gcj02.fromWGS84(output, output, opt_dimension);
    return projts.ll2smerc(output, output, opt_dimension);
  };
  projts.gmerc2smerc = function(input, opt_output, opt_dimension) {
    let output = projts.smerc2ll(input, input, opt_dimension);
    output = gcj02.toWGS84(output, output, opt_dimension);
    return projts.ll2smerc(output, output, opt_dimension);
  };

  projts.ll2smerc = sphericalMercator.forward;
  projts.smerc2ll = sphericalMercator.inverse;
